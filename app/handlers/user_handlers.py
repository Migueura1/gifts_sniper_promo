...
from backend.autobuy import autobuy_test
...
start_kb = create_inline_kb(2, 'enter_stars', 'price_from', 'price_to', 'supply', 'link_channels', 'test_autobuy')
...
############################################################################
@router.callback_query(F.data == 'enter_stars')
async def enter_stars(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(
        text='Сколько звезд?(от 1 до 50к)',
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='Отмена', callback_data='cancel')]])
    )

    await state.set_state(UserStates.fill_stars)


@router.message(StateFilter(UserStates.fill_stars))
async def fill_stars(message: Message, state: FSMContext):
    amount_stars = await validate_amount(message)
    ...
    await message.answer_invoice(
        title="Оплата",
        description='Жми кнопку ниже',
        prices=prices,
        provider_token="",
        payload=f"pay_{amount_stars}",
        currency="XTR"
    )
...
async def on_successful_payment(message: Message):
    logger.info(f"Баланс бота увеличен на: {message.successful_payment.invoice_payload.split('_')[-1]} ⭐️")
    await message.answer(
        text=f"Оплата успешна!\n\nСохраните ID транзакции: <code>{message.successful_payment.telegram_payment_charge_id}</code>",
...
async def start_test(callback: CallbackQuery, bot: Bot):
    await callback.message.edit_text(
        text='<b>Начинаю тестирование..</b>'
    )

    await autobuy_test(bot)
